...
public class TaskController {
    ...
    public ResponseEntity<?> updateTask(
        ...
        if (!existingTask.getUser().getId().equals(user.getId())) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Görev güncellenemez");
        }
        ...
        if (taskRequest.getProjectId() != null) {
            // Yeni proje ataması yapılmak isteniyor
            if (taskRequest.getProjectId() == 0) {
                // 0 gönderilirse proje bağlantısını kaldır
        ...
        // Eğer projectId gönderilmediyse mevcut durumu koru (null veya projeli)
